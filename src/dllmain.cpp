...
HRESULT WINAPI DirectInput8Create(HINSTANCE hinst, DWORD ver, const GUID& riid, void** out, void* unk) {
    ...
    return fn ? fn(hinst, ver, riid, out, unk) : E_FAIL;
}
...
static const uint32_t MACRO_BLK_OFF = 0x192c;   // buttons (+0x192c) + analog (+0x1930..+0x1940)
static const uint32_t MACRO_BLK_SZ  = 0x18;     // the per-frame input the gameplay processor writes
static const int MACRO_MAX_FRAMES   = 1800;     // ~30 s at 60 fps
static uint8_t g_recBuf[MACRO_MAX_FRAMES][MACRO_BLK_SZ];
...
static bool g_liveOverride   = true;            // live input overrides playback
...
static int  g_recKey  = 0x71;                   // VK_F2 (rebindable)
...
extern "C" __attribute__((cdecl)) void macroInputTick(void* objv) {
    ...
    if (st == 1) {                               // recording
        int n = g_recLen;
        if (n < MACRO_MAX_FRAMES) { memcpy(g_recBuf[n], obj + MACRO_BLK_OFF, MACRO_BLK_SZ); g_recLen = n + 1; }
        else g_recState = 0;                     // buffer full -> stop
    } else if (st == 2) {                        // playing
        int i = g_playIdx;
        if (i >= g_recLen) { g_recState = 0; g_playIdx = 0; }
        else {
            if (g_liveOverride && live != 0) {   // player is driving -> let live input through
                if (g_recordOverride) memcpy(g_recBuf[i], obj + MACRO_BLK_OFF, MACRO_BLK_SZ);
            } else {
                memcpy(obj + MACRO_BLK_OFF, g_recBuf[i], MACRO_BLK_SZ);  // replay recorded input
            }
            g_playIdx = i + 1;
        }
    }
...
static void macroToggleRecord() {
    if (g_recState == 1) {
        g_recState = 0;
        int nz = 0; for (int i = 0; i < g_recLen; i++) if (*(uint32_t*)g_recBuf[i] != 0) nz++;
        logf("[macro] record stop: %d frames, %d with buttons, hookFires=%u, lastLive=0x%X",
             g_recLen, nz, g_macroHits, g_macroLive);
    } else { g_recLen = 0; g_playIdx = 0; g_recState = 1; logf("[macro] record start (hookFires=%u)", g_macroHits); }
}
static void macroTogglePlay() {
    if (g_recState == 2) { g_recState = 0; logf("[macro] play stop"); }
    else if (g_recLen > 0) { g_playIdx = 0; g_recState = 2; logf("[macro] play start (%d frames)", g_recLen); }
}
...
static bool applyLadyJC() {
    ...
    uint8_t cave[] = {
        ...
        0x81,0xfa,0x00,0x02,0x00,0x00,                 // [13] cmp edx,200
        0x72,0x28,                                     // [19] jb abort (->61)
        0x81,0xfa,0x30,0x05,0x00,0x00,                 // [21] cmp edx,530
        0x73,0x20,                                     // [27] jae abort (->61)   keep 0x200-0x52F
        0xf3,0x0f,0x10,0x93, 0x2c,0x05,0x00,0x00,      // [29] movss xmm2,[ebx+52C]
        0xf3,0x0f,0x10,0x3d, 0,0,0,0,                  // [37] movss xmm7,[thresh]  abs@41
        0x0f,0x2f,0xd7,                                // [45] comiss xmm2,xmm7
        0x72,0x0b,                                     // [48] jb abort (->61)
        0x83,0xc0,0x02,                                // [50] add eax,2
        0x89,0x81,0x24,0x39,0x00,0x00,                 // [53] mov [ecx+3924],eax  (jump cancel)
        0xeb,0x06,                                     // [59] jmp done (->67)
        0x89,0x81,0xa8,0x39,0x00,0x00,                 // [61] abort: mov [ecx+39A8],eax
        0x0f,0x57,0xd2,                                // [67] done: xorps xmm2,xmm2
        0x0f,0x57,0xff,                                // [70] xorps xmm7,xmm7
        0x5a,                                          // [73] pop edx
        0xE9, 0,0,0,0,                                 // [74] jmp returnhere   rel32@75
        0,0,0,0                                        // [79] thresh (float)
    };
    void* mem = VirtualAlloc(nullptr, sizeof(cave), MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!mem) { logf("[ladyjc] VirtualAlloc failed"); return false; }
    const float thresh = 1.0f;
    memcpy(cave + 79, &thresh, 4);
    uint32_t threshAbs = (uint32_t)((uintptr_t)mem + 79);
    memcpy(cave + 41, &threshAbs, 4);
    // back-jmp rel32 relative to END of the E9 (E9 at offset 74, rel32 field @75).
    int32_t backRel = (int32_t)((site + 6) - ((uintptr_t)mem + 74 + 5));
    memcpy(cave + 75, &backRel, 4);
    memcpy(mem, cave, sizeof(cave));
    FlushInstructionCache(GetCurrentProcess(), mem, sizeof(cave));
    uint8_t hook[6] = { 0xE9, 0,0,0,0, 0x90 };
    int32_t toRel = (int32_t)((uintptr_t)mem - (site + 5));
...
// ===================== Taunt (force a chosen taunt per character) =============
// Each character's taunt dispatch does `push eax(=taunt anim id); call method`. EAX is set
// far upstream, so we cave `mov eax,[&g_tauntId[i]]` in front of the push -> the in-game
// taunt button plays whatever id we pick. Sites verified by disasm:
//   [0] Nero  0x5145C8  `push eax; mov ecx,edi; call edx`          (5 bytes)
//   [1] Dante 0x4DF7C0  `push eax; mov eax,[edx+0x3D4]`            (7 bytes)
//   [2] Vergil 0xD5FF1  `push eax; mov eax,[edx+0x3D4]`            (7 bytes)
//   [3] Lady/Trish 0xCACE4 `push eax; mov eax,[edx+0x3D4]`         (7 bytes, shared)
// id 0 = remove the cave (vanilla taunt cycling). The cave reads the id from our variable,
// so changing the dropdown needs no re-patch.
static volatile uint32_t g_tauntId[4] = {0,0,0,0};      // forced id per site
static const struct { uint32_t rva; uint8_t len; bool nero; } kTaunt[4] = {
    {0x5145C8, 5, true}, {0x4DF7C0, 7, false}, {0xD5FF1, 7, false}, {0xCACE4, 7, false},
};
static void*   g_tauntCave[4] = {nullptr,nullptr,nullptr,nullptr};
static uint8_t g_tauntSaved[4][8];
static bool    g_tauntOn[4]   = {false,false,false,false};
static void stopTauntSite(int i) {
    if (!g_tauntOn[i]) return;
    writeBytes(g_base + kTaunt[i].rva, g_tauntSaved[i], kTaunt[i].len);
    if (g_tauntCave[i]) { VirtualFree(g_tauntCave[i], 0, MEM_RELEASE); g_tauntCave[i] = nullptr; }
    g_tauntOn[i] = false;
}
static void applyTauntSite(int i) {
    if (g_tauntOn[i] || !g_base) return;
    uintptr_t site = g_base + kTaunt[i].rva; uint8_t len = kTaunt[i].len;
    if (!inModule(site, len)) return;
    uint8_t cave[40]; int n = 0;
    uint32_t idAddr = (uint32_t)(uintptr_t)&g_tauntId[i];
    cave[n++]=0xA1; memcpy(cave+n,&idAddr,4); n+=4;                  // mov eax,[&g_tauntId[i]]
    if (kTaunt[i].nero) { cave[n++]=0x50; cave[n++]=0x8B; cave[n++]=0xCF; cave[n++]=0xFF; cave[n++]=0xD2; }  // push eax; mov ecx,edi; call edx
    else { uint8_t o[]={0x50,0x8B,0x82,0xD4,0x03,0x00,0x00}; memcpy(cave+n,o,7); n+=7; }                    // push eax; mov eax,[edx+0x3D4]
    void* mem = VirtualAlloc(nullptr, 64, MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!mem) return;
    int32_t backRel = (int32_t)((site + len) - ((uintptr_t)mem + n + 5));
    cave[n++]=0xE9; memcpy(cave+n,&backRel,4); n+=4;                 // jmp back to site+len
    memcpy(mem, cave, n); FlushInstructionCache(GetCurrentProcess(), mem, n);
    memcpy(g_tauntSaved[i], (void*)site, len);
    uint8_t hook[8]; hook[0]=0xE9;
    int32_t toRel = (int32_t)((uintptr_t)mem - (site + 5));
    memcpy(hook+1,&toRel,4);
    for (int k=5;k<len;k++) hook[k]=0x90;
    writeBytes(site, hook, len);
    g_tauntCave[i] = mem; g_tauntOn[i] = true;
    logf("[taunt] site %d hooked -> cave %p", i, mem);
}
static void setTaunt(int i, uint32_t id) {
    g_tauntId[i] = id;
    if (id == 0) stopTauntSite(i);
    else         applyTauntSite(i);     // idempotent: installs once, then just updates the var
}
...
static void DrawUI() {
    ...
    if (beginBlackTab(tr("Character###s4","角色###s4"))) {
        ...
        if (ImGui::CollapsingHeader("Taunt (per character)", ImGuiTreeNodeFlags_DefaultOpen)) {
            ImGui::Indent(8.0f);
            ImGui::TextDisabled("Pick a taunt; the in-game Taunt button then plays it.");
            static const uint32_t idNero[]  = {0x96,0x97,0x98,0x99,0x9A,0x9B,0x9C,0x9D,0x9E,0xA5,0xA6};
            static const uint32_t idDante[] = {0x96,0x97,0x98,0x99,0x9A,0x9B,0x9C,0x9D,0x9E,0x9F,0xA0,0xA1,0xA6,0xA7,0xA8,0xA9,0xAA,0xAB,0xAC,0xAD,0xAE};
            static const uint32_t idVergil[]= {0x96,0x97,0x98,0x99,0x9E,0xA6};
            static const uint32_t idLady[]  = {0x96,0x98,0x9E,0x9F,0x97,0x99,0x9A,0x9B,0x9C,0x9D};
            static const uint32_t idTrish[] = {0x96,0x98,0x9E,0x9F,0x97,0x99,0x9A,0x9B,0x9C,0x9D,0xA0,0xA1,0xA6,0xA7,0xA8,0xA9,0xAA,0xAB,0xAC,0xAD,0xAE};
            struct TC { const char* name; int site; const uint32_t* ids; int count; };
            static const TC tcs[5] = {
                {"Nero",0,idNero,11},{"Dante",1,idDante,21},{"Vergil",2,idVergil,6},{"Lady",3,idLady,10},{"Trish",4,idTrish,21},
            };
            static int sel[5] = {0,0,0,0,0};
            for (int c = 0; c < 5; c++) {
                int site = (c==4)?3:c;     // Trish shares Lady's site (3)
                char preview[16]; if (sel[c]==0) snprintf(preview,sizeof preview,"Off"); else snprintf(preview,sizeof preview,"Taunt %d",sel[c]);
                char cid[24]; snprintf(cid,sizeof cid,"%s##taunt%d",tcs[c].name,c);
                ImGui::SetNextItemWidth(150);
                if (ImGui::BeginCombo(cid, preview)) {
                    if (ImGui::Selectable("Off", sel[c]==0)) { sel[c]=0; setTaunt(site,0); }
                    for (int k=1;k<=tcs[c].count;k++) {
                        char it[16]; snprintf(it,sizeof it,"Taunt %d",k);
                        if (ImGui::Selectable(it, sel[c]==k)) { sel[c]=k; setTaunt(site, tcs[c].ids[k-1]); }
                    }
                    ImGui::EndCombo();
                }
            }
            ImGui::TextDisabled("Lady & Trish share a slot; only the active one plays.");
            ImGui::Unindent(8.0f);
        }
